...
function crearCodigoReglas (regla, arreglo, tabs, variable, proyeccion, posProyeccion, posVariable, posSubVariable) {
    ...
    if(regla.campoObjetivo.indexOf('COLUMNA') == 0) {
        if(esCondicion) {
            ...
            arreglo.push({codigo: "\n"+tabsText+"sumarSaldoOriginal(arregloDepositos"+idFiltro+"[i].saldo, "+posProyeccion+", "+posVariable+", "+posSubVariable+", -1, arregloDepositos"+idFiltro+"[i].moneda.toLowerCase(), '');", filtro: regla.filtro});
        ...
        arreglo.push({codigo: "\n"+tabsText+"var temp = totalDeposito * "+(factorValor)+";", filtro: regla.filtro});
        if(calcularDepoPresta) {
            arreglo.push({codigo: "\n\t"+tabsText+"insertAccount(arregloDepositos"+idFiltro+"[i], false, "+proyeccion+", 2, temp, '"+(variable+proyeccion)+"', "+posProyeccion+");", filtro: regla.filtro});
        }
        if(calcularContables) {
            arreglo.push({codigo: "\n\t"+tabsText+"insertVarContable(arregloDepositos"+idFiltro+"[i].moneda, 2, arregloDepositos"+idFiltro+"[i], "+proyeccion+", temp, "+posProyeccion+");", filtro: regla.filtro});
        }
        if(calcularClientes) {
            arreglo.push({codigo: "\n\t"+tabsText+"insertCliente(arregloDepositos"+idFiltro+"[i].moneda, arregloDepositos"+idFiltro+"[i], false, "+proyeccion+", 2, '"+(variable+proyeccion)+"', temp, "+posProyeccion+");", filtro: regla.filtro});
        }
        arreglo.push({codigo: "\n"+tabsText+"sumarSaldoOriginal(arregloDepositos"+idFiltro+"[i].saldo, "+posProyeccion+", "+posVariable+", "+posSubVariable+", -1, arregloDepositos"+idFiltro+"[i].moneda.toLowerCase(), '');", filtro: regla.filtro});
    ...
    if(regla.valor.indexOf('LISTA') == 0) {
        ...
        if(esCondicion) {
            ...
            arreglo.push({codigo: "\n\t"+tabsText+"var  temp = arregloDepositos"+idFiltro+"[i].saldo * "+(factorValor)+";", filtro: regla.filtro});
            if(calcularDepoPresta) {
                arreglo.push({codigo: "\n"+tabsText+"insertAccount(arregloDepositos"+idFiltro+"[i], false, "+proyeccion+", "+2+", temp, '"+(variable+proyeccion)+"', "+posProyeccion+");", filtro: regla.filtro});
            }
            if(calcularContables) {
