...
export default class EditarDashboard extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            seccionesDashboard: [],
        ...
        this.getDashboardSections = this.getDashboardSections.bind(this);
        this.modificarDashboard = this.modificarDashboard.bind(this);
        this.eliminarSeccionDashboardModificar = this.eliminarSeccionDashboardModificar.bind(this);
        this.guardarSeccionesDashboard = this.guardarSeccionesDashboard.bind(this);
        ...
        this.eliminarDashboard = this.eliminarDashboard.bind(this);
    }
    ...
    getDashboard () {
        ...
        transaction.begin(err => {
            ...
            request.query("select * from Dashboard where ID = "+this.props.dashboardSeleccionado.ID, (err, result) => {
                if (err) {
                    if (!rolledBack) {
                        console.log(err);
    ...
    modificarDashboard () {
        ...
        if(nombre.length > 0) {
            ...
            transaction.begin(err => {
                ...
                const request = new sql.Request(transaction);
                request.query("update Dashboard set nombre = '"+nombre+"', descripcion = '"+descripcion+"'", (err, result) => {
                    if (err) {
                        console.log(err);
                        if (!rolledBack) {
                            transaction.rollback(err => {
                        ...
                        transaction.commit(err => {
                            this.eliminarSeccionDashboardModificar();
                            var self = this;
                            setTimeout(function () {
    ...
    eliminarSeccionDashboardModificar () {
        ...
        transaction1.begin(err => {
            ...
            request1.query("delete from SeccionDashboard where dashboardID = "+this.props.dashboardSeleccionado.ID, (err, result) => {
                if (err) {
                    if (!rolledBack) {
                        console.log(err);
    ...
    guardarSeccionesDashboard () {
        for (var i = 0; i < this.state.seccionesDashboard.length; i++) {
            ...
            transaction.begin(err => {
                ...
                const request = new sql.Request(transaction);
                request.query("insert into SeccionDashboard (dashboardID, tamano, tipoObjeto, instruccion) values("+this.props.dashboardSeleccionado.ID+", '"+tamano+"', '"+tipoObjeto+"', '"+instruccion+"') ", (err, result) => {
                    if (err) {
                        console.log(err);
                        if (!rolledBack) {
                            transaction.rollback(err => {
            ...
            if(tipoObjeto.length < 10) {
                if( (tipoObjeto.localeCompare("grafica") == 0 && this.state.tipoGraficoUpdate[index].length > 0) || tipoObjeto.localeCompare("tabla") == 0) {
                    ...
                    if( (this.state.tipoGraficoUpdate[index] != undefined && ((this.state.tipoGraficoUpdate[index].localeCompare("LINEA") == 0 || this.state.tipoGraficoUpdate[index].localeCompare("AREA") == 0 || 
                        ...
                        (this.state.tipoObjetoUpdate[index].localeCompare("tabla") == 0 && this.state.variablesSeleccionadasSeccionesDashboardTablaUpdate[index].length > 0) ) {
                        var instruccion = '';
                        if (tipoObjeto.localeCompare("grafica") == 0) {
    ...
    eliminarDashboard () {
        ...
        transaction.begin(err => {
            ...
            const request = new sql.Request(transaction);
            request.query("delete from Dashboard where ID = "+this.props.dashboardSeleccionado.ID, (err, result) => {
                if (err) {
                    if (!rolledBack) {
                        console.log(err);
                        transaction.rollback(err => {
                        ...
                        <div className={"card influencer-profile-data"}>
                            <div className={"card-body"}>
                                <div className={"border-top border-bottom addPaddingToConfig"} style={{width: "100%"}}>
                                    ...
                                    <hr/>
                                    <div className={"row"} style={{display: "flex", alignItems: "center", justifyContent: "center"}}>
                                        <a className={"btn btn-success btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa"}} onClick={this.modificarDashboard}>Modificar</a>
                                        <a className={"btn btn-danger btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa"}} onClick={this.eliminarDashboard}>Eliminar</a>
                                    </div>
                                    <br/>
                                </div>
                            </div>
