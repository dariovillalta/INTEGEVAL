...
var peso = 0, idFormula = '', nombre = '', formula = '', nombreEncargadoRiesgo = '', primeraVezCargado = true;
...
export default class EditarRiesgo extends React.Component {
    constructor(props) {
        ...
        this.eliminarRiesgo = this.eliminarRiesgo.bind(this);
    ...
    guardarRiesgo () {
        ...
        if(idFormula.length > 0 && idFormula.length < 101) {
                        ...
                        if(responsable.length > 0) {
                            if( !isNaN(parseInt(peso)) ) {
                                ...
                                transaction.begin(err => {
                                    ...
                                    const request = new sql.Request(transaction);
                                    request.query("update Riesgos set idFormula = '"+idFormula+"' nombre = '"+nombre+"', peso = "+peso+", formula = '"+formula+"', responsable = '"+responsable+"' where ID = "+this.props.idRiesgoSeleccionado, (err, result) => {
                                        if (err) {
                                            console.log(err);
                                            if (!rolledBack) {
                                                transaction.rollback(err => {
            ...
            alert("el identificador en fórmula del riesgo debe tener una longitud mayor a 0 y menor a 101");
        }
    }
    ...
    */
    ...
    guardarVariablesCalculadas () {
        ...
        for (var a = 0; a < arregloDeRiesgos.length; a++) {
            if (this.props.idRiesgoSeleccionado == arregloDeRiesgos[a].ID) {
    ...
    guardarResultadosNombreRiesgo (riesgo, fechaNombreTabla) {
        ...
        this.saveBitacora(hoy, "Usuario: "+this.props.userName+" realizo el cálculo para el riesgo: "+riesgo.nombre);
    }
    ...
    */
    ...
    eliminarRiesgo () {
        ...
        transaction1.begin(err => {
            ...
            request2.query("DELETE FROM Riesgos WHERE ID = "+this.props.idRiesgoSeleccionado, (err, result) => {
                if (err) {
                    console.log(err);
                    if (!rolledBack) {
                    ...
                    transaction1.commit(err => {
                        ...
                        this.props.getRiesgos();
                        this.props.retornoSeleccionRiesgo();
                    });
                }
            });
        }); // fin transaction1
    ...
    goToTimeline (esExcel) {
        ...
        if(esExcel && idVariableExcel != undefined) {
            this.setState({
                navbar: navbar,
                componentActual: "timelineVariable",
            });
        } else {
            this.setState({
                navbar: navbar,
    ...
    render() {
        if(this.state.componentActual.localeCompare("EditarRiesgo") == 0) {
            return (
                <div>
                    ...
                    <div className={"row"} style={{display: this.props.permision.riesgo.indexOf("V") > -1 || this.props.permision.riesgo.indexOf("E") > -1 ? "" : "none" }}>
                        <div className={"col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12"}>
                            <div className={"card influencer-profile-data"}>
                                <div className={"card-body"}>
                                    <div className={"border-top border-bottom addPaddingToConfig"} style={{width: "100%"}}>
                                        ...
                                        <div className={"row"} style={{width: "100%"}}>
                                            <div className={"col-xl-3 col-lg-3 col-md-3 col-sm-3 col-3 form-group"}>
                                                <label htmlFor="idFormula" className="col-form-label">Nombre Riesgo</label>
                                            </div>
                                            <div className={"col-xl-9 col-lg-9 col-md-9 col-sm-9 col-9 form-group"}>
                                        ...
                                        <a className={"btn btn-warning btn-block btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa", display: this.props.permision.riesgo.indexOf("E") > -1 ? "" : "none"}} onClick={this.goCrearUmbral}>Umbrales</a>
                                        <br/>
                                        <div className={"row"} style={{display: "flex", alignItems: "center", justifyContent: "center"}}>
                                            <a className={"btn btn-brand btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa", display: this.props.permision.riesgo.indexOf("E") > -1 ? "" : "none"}} onClick={this.guardarRiesgo}>Modificar Riesgo</a>
                                            <a className={"btn btn-secondary btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa", marginLeft: "10px", display: this.props.permision.riesgo.indexOf("E") > -1 ? "" : "none"}} onClick={this.eliminarRiesgo}>Eliminar Riesgo</a>
                                        </div>
                                        <br/>
                                        <div className={"row"} style={{display: "flex", alignItems: "center", justifyContent: "center", display: this.props.permision.riesgo.indexOf("E") > -1 ? "" : "none"}}>
                                            <a className={"btn btn-primary btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa", marginLeft: "10px", display: this.props.permision.riesgo.indexOf("E") > -1 ? "" : "none"}} onClick={this.traerArchivosExcel}>Realizar Cálculo</a>
                                            <a className={"btn btn-info btnWhiteColorHover font-bold font-20"} style={{color: "#fafafa", marginLeft: "10px"}} onClick={() => this.goToTimeline(false)}>Historial de Variable</a>
                                        </div>
                                        <br/>
                                    </div>
                                </div>
        ...
        } else if(this.state.componentActual.localeCompare("timelineVariable") == 0) {
            return (
                <div>
                    <TimelineVariable pool={this.props.pool}
                                            variable={{ID: this.props.idRiesgoSeleccionado, esObjeto: false, esInstruccionSQL: false, esColeccion: false}}
                                            esVariable={false}
                                            esIndicador={false}
                                            tablaInstruccion={"ResultadosNombreRiesgos where nombreRiesgo = '"+this.props.nombreRiesgo+"' "}
