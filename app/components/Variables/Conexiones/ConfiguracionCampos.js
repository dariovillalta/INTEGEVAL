  ...
  */
  ...
  _createClass(ConfiguracionCampos, [{
              ...
              if (tipoCampo.length > 0 && tipoCampo.length < 26) {
                if (guardarCampo != undefined) {
                  ...
                  transaction.begin(function (err) {
                    ...
                    var request = new _mssql["default"].Request(transaction);
                    request.query("insert into Campos (tablaID, nombre, tipo, guardar, formula) values (" + idTabla + ",'" + campoNombre + "','" + tipoCampo + "','" + guardarCampo + "','')", function (err, result) {
                      if (err) {
                        if (!rolledBack) {
                          console.log(err);
                          transaction.rollback(function (err) {});
    ...
    value: function render() {
      ...
      }, "Configuraci\xF3n de campos a traer de la tabla ", this.props.nombreTablaSeleccionada), _react["default"].createElement("div", {
